...
<div class="card basic-data-table">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table bordered-table mb-0" id="dataTable" data-page-length='25'>
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Label</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                @if($item->type === 'image')
                                    <img src="{{ asset('storage/'.$item->value) }}" alt="{{ $item->label }}" class="img-fluid" style="max-height: 50px;">
                                @else
                                    <h6 class="text-md mb-0 fw-medium flex-grow-1">
                                        {{ strip_tags(\Illuminate\Support\Str::limit($item->value, 50)) }}
                                    </h6>
                                @endif
                            </div>
                        </td>
                        <td>
                            @can($routeBase . '.edit')
                            <a href="{{ route($routeBase . '.editValue', $item->id) }}"
